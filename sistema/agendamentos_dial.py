...
def confirmar(self):
    pet_nome = self.comboPet.currentText()
    pet_id = self.pets_dict.get(pet_nome)
    dono_email = self.usuarios_dict.get(self.comboDono.currentText())
    ...
    try:
        ...
        # Enviar email
        enviar_email_confirmacao_consulta(dono_email, pet_nome.split(" (")[0], data_consulta, hora_consulta, nome_vet)
